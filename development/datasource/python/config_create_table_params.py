    ...
    if 'subject' in df_filters_req:
        if 'daily' in df_time_sel:
            ...
            for filter, var, var_grp in zip(filters, filter_var_footer, filter_grp):
                if student_id_exists is True:
                    dml_queries += '{"' + filter + '_footer_grade_subject_daily":"select grade,subject,' + var + ',sum(total_schools) as total_schools,sum(students_count) as students_count,' + date_col + ',week,academic_year from ' + table_names + '_grade_sub_sch_footer_daily a  group by grade,subject,week,academic_year,' + date_col + ',' + var + '"},'
                    dml_queries += '{"' + filter + '_management_footer_grade_subject_daily":"select grade,subject,' + var + ',school_management_type,sum(total_schools) as total_schools,sum(students_count) as students_count,' + date_col + ',week,academic_year from ' + table_names + '_grade_sub_sch_footer_daily a group by grade,subject,week,academic_year,school_management_type,' + date_col + ',' + var + '"},'
                else:
                    dml_queries += '{"' + filter + '_footer_grade_subject_daily":"select grade,subject,' + var + ',sum(total_schools) as total_schools,' + date_col + ',week,academic_year from ' + table_names + '_grade_sub_sch_footer_daily a  group by grade,subject,week,academic_year,' + date_col + ',' + var + '"},'
                    dml_queries += '{"' + filter + '_management_footer_grade_subject_daily":"select grade,subject,' + var + ',school_management_type,sum(total_schools) as total_schools,' + date_col + ',week,academic_year from ' + table_names + '_grade_sub_sch_footer_daily a group by grade,subject,week,academic_year,school_management_type,' + date_col + ',' + var + '"},'
        if 'weekly' in df_time_sel:
        ...
        if 'last_day' in df_time_sel:
            ...
            if student_id_exists is True:
                ...
                to_sql += 'create or replace view ' + table_names + "_school_stds_cnt_grade_sub_last_day as  select concat('Grade_',  grade) as grade,subject,shd.school_id,cluster_id,block_id,district_id,count(distinct student_id) as students_count,count(distinct school_id) as total_schools from " + table_names + '_trans a  join school_hierarchy_details shd on a.school_id= shd.school_id ' + last_day_filter + condition_val +  ' group by shd.school_id,cluster_id,block_id,district_id,grade,subject;' + '\n'
