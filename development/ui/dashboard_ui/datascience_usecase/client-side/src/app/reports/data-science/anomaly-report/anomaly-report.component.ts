...
import { AnomalyReportService } from "../../../services/anomaly-report.service";
...
@Component({
  ...
  styleUrls: ['./anomaly-report.component.css'],
...
export class AnomalyReportComponent implements OnInit {
  ...
  colorGenData: any = [];
  reportName = "anomaly_access_by_location";
  dateAndTime;

  anomaly_type_list = [
    { key: "percentile_score", value: "Anomaly 1" },
    { key: "maximum_score", value: "Anomaly 2" },
    { key: "minimum_score", value: "Anomaly 3" }
  ];
  constructor(
    public http: HttpClient,
    public service: AnomalyReportService,
  ...
  ngOnInit() {
    ...
    this.globalService.latitude = this.lat = this.commonService.mapCenterLatlng.lat;
  ...
  getDistricts(): void {
    this.service.anomalyMapDistWise({ management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe((res) => {
      this.data = res["data"];
  ...
  getBlocks(distId, blockId?: any): void {
    this.service.anomalyMapBlockWise(distId, { management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe((res) => {
      this.data = res["data"];
  ...
  getClusters(distId, blockId, clusterId): void {
    this.service.anomalyMapClusterWise(distId, blockId, { management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe((res) => {
      this.data = res["data"];
  ...
  districtWise() {
    try {
      ...
      if (this.myDistData != undefined) {
        ...
        this.gettingAnomalyFilters(this.data);
        ...
        this.myData = this.service.anomalyMapDistWise({ management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe(
          (res) => {
            this.myDistData = res;
            this.data = res["data"];
            this.gettingAnomalyFilters(this.data);
            ...
            this.data.sort((a, b) =>
              `${a[this.anomalyData]}` > `${b[this.anomalyData]}`
                ? 1
                : `${b[this.anomalyData]}` > `${a[this.anomalyData]}`
                  ? -1
                  : 0
            );
            this.genericFun(this.myDistData, options, this.fileName);
  ...
  blockWise() {
    try {
      ...
      this.myData = this.service.anomalyMapAllBlockWise({ management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe(
        (res) => {
  ...
  clusterWise() {
    try {
      ...
      this.myData = this.service.anomalyMapAllClusterWise({ management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe(
        (res) => {
          this.data = res["data"];
          this.gettingAnomalyFilters(this.data);
          let options = {
  ...
  schoolWise() {
    try {
      ...
      this.myData = this.service.anomalyMapAllSchoolWise({ management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe(
        (res) => {
          this.data = res["data"];
          this.gettingAnomalyFilters(this.data);
          let options = {
  ...
  onDistrictSelect(districtId) {
    ...
    this.myData = this.service.anomalyMapBlockWise(districtId, { management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe(
      (res) => {
        this.data = res["data"];
        this.gettingAnomalyFilters(this.data);
  ...
  onBlockSelect(blockId) {
    ...
    this.myData = this.service
      .anomalyMapClusterWise(this.districtHierarchy.distId, blockId, { management: this.management, category: this.category, anomaly_type: this.anomaly_type })
      .subscribe(
        (res) => {
          this.data = res["data"];
          this.gettingAnomalyFilters(this.data);
  ...
  onClusterSelect(clusterId) {
    ...
    this.myData = this.service.anomalyMapAllBlockWise({ management: this.management, category: this.category, anomaly_type: this.anomaly_type }).subscribe(
      (result: any) => {
        this.myData = this.service
          .anomalyMapSchoolWise(
            this.blockHierarchy.distId,
            this.blockHierarchy.blockId,
            clusterId, { management: this.management, category: this.category, anomaly_type: this.anomaly_type }
          )
          .subscribe(
            (res) => {
              this.data = res["data"];
  ...
  gettingAnomalyFilters(data) {
    ...
    for (var i = 0; i < Object.keys(data[0].metrics).length; i++) {
      ...
      this.anomalyFilter.push({
        key: Object.keys(this.data[0].metrics)[i],
        value: val,
      });
    }

    this.anomalyFilter.unshift({
      key: "anomaly_percentage",
      value: "Anomaly Score",
    });

    var anomalyKey = this.anomalyFilter.filter(function (obj) {
      return obj.key == "anomaly_percentage";
    });

    this.anomalyFilter = this.anomalyFilter.filter(function (obj) {
      return obj.key !== "anomaly_percentage";
    });

    this.anomalyFilter.sort((a, b) =>
      a.value > b.value ? 1 : b.value > a.value ? -1 : 0
    );
    this.anomalyFilter.splice(0, 0, anomalyKey[0]);
  }

  public anomalyData = "anomaly_percentage";
  public level = "District";

  onanomalySelect(data) {
    this.anomaly_type = data;
    this.levelWiseFilter();
  }
  ...
  generateToolTip(markers, level, markerIcon, lat, lng) {
    ...
    var anomalyName = this.anomalyData;
    ...
    if (level != "School" || level != "schoolPerCluster") {
      ...
      yourData1 = this.globalService
        .getInfoFrom(detailSchool, "", level, "anomaly-map", anomalyName, colorText)
        .join(" <br>");
    } else {
      yourData1 = this.globalService
        .getInfoFrom(orgObject, "", level, "anomaly-map", anomalyName, colorText)
        .join(" <br>");
    }
    ...
    var yourData = this.globalService
      .getInfoFrom(ordered, "", level, "anomaly-map", anomalyName, colorText)
      .join(" <br>");
    ...
    const popup = R.responsivePopup({
      ...
      "<br>" +
      yourData1 +
      "<br><br><b><u>School Anomaly Metrics (% of schools)</u></b>" +
      "<br>" +
      yourData
    );
  ...
  getDownloadableData(markers, level) {
    ...
    if (level == "District") {
      if (this.anomalyData !== "anomaly_percentage") {
        let obj = {
          district_id: markers.details.district_id,
          district_name: markers.details.district_name,
          [this.anomalyData]: markers.metrics[`${this.anomalyData}`] + "%",
        };
