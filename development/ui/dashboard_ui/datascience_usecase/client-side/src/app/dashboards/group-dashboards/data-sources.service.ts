  ...
  constructor(public service: AppServiceComponent) {
    service.getDataSource().subscribe((res: any) => {
      res.forEach((element) => {
        ...
        if (element.template == "anomaly") {
          this.dataSources.anomaly = element.status;
        }
        if (element.template == "dropout") {
          this.dataSources.dropout = element.status;
        }
        ...
        if (element.template === 'datascience') {
          this.dataSources.datascience = element.status;
        }
