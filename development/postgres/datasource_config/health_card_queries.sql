/* Insert statements to health card config  */
/* District */
...
on conflict on constraint health_card_config_pkey do nothing;

insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('attendance','row_to_json(student_attendance.*) as student_attendance',
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('semester','row_to_json(semester.*) as student_semester',
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('udise','row_to_json(udise.*) as udise',
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('pat','row_to_json(pat.*) as PAT_performance',
'left join (select ped.*,pes.value_below_33,pes.value_between_33_60,pes.value_between_60_75,pes.value_above_75 from 
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('crc','row_to_json(crc.*) as CRC_visit',
'left join hc_crc_district as crc
on basic.district_id=crc.district_id',True,'district','overall')
on conflict on constraint health_card_config_pkey do nothing;

insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('infra','row_to_json(infra.*) as school_infrastructure',
...
/* health card last 30 days  insert statements */
/* district */
...
/* Function to create health card views for overall and last 30 days at different levels */

CREATE OR REPLACE FUNCTION health_card_create_views()
RETURNS text AS
$$
DECLARE
select_query_dist text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''district'' and time_period=''overall''  order by id)as d';   
select_cols_dist text;
join_query_dist text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''district'' and time_period=''overall''  order by id)as d';
join_cols_dist text;
district_view text;
select_query_blk text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''block'' and time_period=''overall''  order by id)as d';   
select_cols_blk text;
join_query_blk text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''block'' and time_period=''overall''  order by id)as d';
join_cols_blk text;
block_view text;
select_query_cst text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''cluster'' and time_period=''overall''  order by id)as d';   
select_cols_cst text;
join_query_cst text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''cluster'' and time_period=''overall''  order by id)as d';
join_cols_cst text;
cluster_view text;
select_query_scl text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''school'' and time_period=''overall''  order by id)as d';   
select_cols_scl text;
join_query_scl text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''school'' and time_period=''overall''  order by id)as d';
join_cols_scl text;
school_view text;

select_query_dist_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''district'' and time_period=''last30''  order by id)as d';   
select_cols_dist_last30 text;
join_query_dist_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''district'' and time_period=''last30''  order by id)as d';
join_cols_dist_last30 text;
district_view_last30 text;
select_query_blk_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''block'' and time_period=''last30''  order by id)as d';   
select_cols_blk_last30 text;
join_query_blk_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''block'' and time_period=''last30''  order by id)as d';
join_cols_blk_last30 text;
block_view_last30 text;
select_query_cst_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''cluster'' and time_period=''last30''  order by id)as d';   
select_cols_cst_last30 text;
join_query_cst_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''cluster'' and time_period=''last30''  order by id)as d';
join_cols_cst_last30 text;
cluster_view_last30 text;
select_query_scl_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''school'' and time_period=''last30''  order by id)as d';   
select_cols_scl_last30 text;
join_query_scl_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''school'' and time_period=''last30''  order by id)as d';
...
district_view='create or replace view health_card_index_district_overall as 
'||select_cols_dist||' '||join_cols_dist||';
';
Execute district_view; 
block_view='create or replace view health_card_index_block_overall as 
'||select_cols_blk||' '||join_cols_blk||';
';
Execute block_view; 
cluster_view='create or replace view health_card_index_cluster_overall as 
'||select_cols_cst||' '||join_cols_cst||';
';
Execute cluster_view; 
school_view='create or replace view health_card_index_school_overall as 
'||select_cols_scl||' '||join_cols_scl||';
';
Execute school_view; 

district_view_last30='create or replace view health_card_index_district_last30 as 
'||select_cols_dist_last30||' '||join_cols_dist_last30||';
';
Execute district_view_last30; 
block_view_last30='create or replace view health_card_index_block_last30 as 
'||select_cols_blk_last30||' '||join_cols_blk_last30||';
';
Execute block_view_last30; 
cluster_view_last30='create or replace view health_card_index_cluster_last30 as 
'||select_cols_cst_last30||' '||join_cols_cst_last30||';
';
Execute cluster_view_last30; 
school_view_last30='create or replace view health_card_index_school_last30 as 
...
/* Health card management queries */
/* district */
...
/* Function to create health card views for overall and last 30 days at different levels with management  */

CREATE OR REPLACE FUNCTION health_card_create_mgmt_views()
RETURNS text AS
$$
DECLARE
select_query_dist_mgmt text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''district_mgmt'' and time_period=''overall''  order by id)as d';   
select_cols_dist_mgmt text;
join_query_dist_mgmt text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''district_mgmt'' and time_period=''overall''  order by id)as d';
join_cols_dist_mgmt text;
district_view_mgmt text;
select_query_blk_mgmt text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''block_mgmt'' and time_period=''overall''  order by id)as d';   
select_cols_blk_mgmt text;
join_query_blk_mgmt text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''block_mgmt'' and time_period=''overall''  order by id)as d';
join_cols_blk_mgmt text;
block_view_mgmt text;
select_query_cst_mgmt text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''cluster_mgmt'' and time_period=''overall''  order by id)as d';   
select_cols_cst_mgmt text;
join_query_cst_mgmt text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''cluster_mgmt'' and time_period=''overall''  order by id)as d';
join_cols_cst_mgmt text;
cluster_view_mgmt text;
select_query_scl_mgmt text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''school_mgmt'' and time_period=''overall''  order by id)as d';   
select_cols_scl_mgmt text;
join_query_scl_mgmt text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''school_mgmt'' and time_period=''overall''  order by id)as d';
join_cols_scl_mgmt text;
school_view_mgmt text;

select_query_dist_mgmt_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''district_mgmt'' and time_period=''last30''  order by id)as d';   
select_cols_dist_mgmt_last30 text;
join_query_dist_mgmt_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''district_mgmt'' and time_period=''last30''  order by id)as d';
join_cols_dist_mgmt_last30 text;
district_view_mgmt_last30 text;
select_query_blk_mgmt_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''block_mgmt'' and time_period=''last30''  order by id)as d';   
select_cols_blk_mgmt_last30 text;
join_query_blk_mgmt_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''block_mgmt'' and time_period=''last30''  order by id)as d';
join_cols_blk_mgmt_last30 text;
block_view_mgmt_last30 text;
select_query_cst_mgmt_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''cluster_mgmt'' and time_period=''last30''  order by id)as d';   
select_cols_cst_mgmt_last30 text;
join_query_cst_mgmt_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''cluster_mgmt'' and time_period=''last30''  order by id)as d';
join_cols_cst_mgmt_last30 text;
cluster_view_mgmt_last30 text;
select_query_scl_mgmt_last30 text:='select string_agg(col,'','') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''school_mgmt'' and time_period=''last30''  order by id)as d';   
select_cols_scl_mgmt_last30 text;
join_query_scl_mgmt_last30 text:='select string_agg(col,'' '') from (select concat(join_query)as col from health_card_config where status=''true'' and category=''school_mgmt'' and time_period=''last30''  order by id)as d';
...
district_view_mgmt='create or replace view health_card_index_district_mgmt_overall as 
'||select_cols_dist_mgmt||' '||join_cols_dist_mgmt||';
';
Execute district_view_mgmt; 
block_view_mgmt='create or replace view health_card_index_block_mgmt_overall as 
'||select_cols_blk_mgmt||' '||join_cols_blk_mgmt||';
';
Execute block_view_mgmt; 
cluster_view_mgmt='create or replace view health_card_index_cluster_mgmt_overall as 
'||select_cols_cst_mgmt||' '||join_cols_cst_mgmt||';
';
Execute cluster_view_mgmt; 
school_view_mgmt='create or replace view health_card_index_school_mgmt_overall as 
'||select_cols_scl_mgmt||' '||join_cols_scl_mgmt||';
';
Execute school_view_mgmt; 

district_view_mgmt_last30='create or replace view health_card_index_district_mgmt_last30 as 
'||select_cols_dist_mgmt_last30||' '||join_cols_dist_mgmt_last30||';
';
Execute district_view_mgmt_last30; 
block_view_mgmt_last30='create or replace view health_card_index_block_mgmt_last30 as 
'||select_cols_blk_mgmt_last30||' '||join_cols_blk_mgmt_last30||';
';
Execute block_view_mgmt_last30; 
cluster_view_mgmt_last30='create or replace view health_card_index_cluster_mgmt_last30 as 
'||select_cols_cst_mgmt_last30||' '||join_cols_cst_mgmt_last30||';
';
Execute cluster_view_mgmt_last30; 
school_view_mgmt_last30='create or replace view health_card_index_school_mgmt_last30 as 
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('basic',' select ''basic_details'' as data_source,row_to_json(basic_details)::text as values  from 
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('attendance',' union
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('semester',' union
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('udise',' union
(select ''udise''as data,row_to_json(udise)::text from (select * from hc_udise_state)as udise) ',
' ',True,'state','overall')
on conflict on constraint health_card_config_pkey do nothing;

insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('pat',' union
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('crc',' union
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('infra',' union
...
/* Function to create health card state level view for last30 */
...
select_query_state_last30 text:='select string_agg(col,'' '') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''state'' and time_period=''last30''  order by id)as d';   
select_cols_state_last30 text;

BEGIN
if EXISTS (select * from health_card_config where status='True' and data_source='infra')  then
...
create_state_view='create or replace view health_card_index_state as '||select_cols_state||'';
Execute create_state_view; 

Execute select_query_state_last30 into select_cols_state_last30;
create_state_view_last30='create or replace view health_card_index_state_last30 as '||select_cols_state_last30||'';
...
/* Health Card index mgmt overall state */

insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('basic',' select ''basic_details'' as data_source,school_management_type,row_to_json(basic_details)::text as values  from 
...
/* Health Card index mgmt last30 state */
...
/* Health Card index overall state mgmt */

create or replace function health_card_index_state_mgmt()
RETURNS text AS
$$
DECLARE
udise_query text:= 'select string_agg(''cast(avg(''||lower(column_name)||'')as int)as ''||lower(column_name),'','')
...
select_cols_state text;
select_query_state_last30 text:='select string_agg(col,'' '') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''state_mgmt'' and time_period=''last30''  order by id)as d';   
...
Execute select_query_state into select_cols_state;
create_state_view=
'create or replace view health_card_index_state_mgmt_overall as '||select_cols_state||'';

Execute create_state_view; 
Execute select_query_state_last30 into select_cols_state_lat30;
create_state_view_last30=
'create or replace view health_card_index_state_mgmt_last30 as '||select_cols_state_lat30||'';
...
update health_card_config set status= true where lower(data_source) in (select split_part(lower(template),'_',2) from nifi_template_info where status=true);
update health_card_config set status= false where lower(data_source) in (select split_part(lower(template),'_',2) from nifi_template_info where status=false);

select health_card_create_views();
select health_card_create_mgmt_views();
select health_card_index_state();
select health_card_index_state_mgmt();
