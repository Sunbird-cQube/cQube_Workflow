...
if EXISTS (select from nifi_template_info where status=true and template='nifi_infra' )  then
...
create_state_view=
'create or replace view health_card_index_state as '||select_cols_state||'';
...
select_query_state text:='select string_agg(col,'' '') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''state'' and time_period=''last30''  order by id)as d';   
select_cols_state text;

create_state_view text;
BEGIN
if EXISTS (select split_part(lower(template),'_',2) from nifi_template_info where status=true and template='nifi_udise' )  then
Execute udise_query into udise_cols;
END IF;
...
Execute select_query_state into select_cols_state;
create_state_view=
'create or replace view health_card_index_state_last30 as '||select_cols_state||'';
...
insert into health_card_config(data_source,select_query,join_query,status,category,time_period) values('udise',' union
(select ''udise''as data,school_management_type,row_to_json(udise)::text from (select sum(total_schools) as total_schools
  ,''||udise_cols||'',    cast(avg(infrastructure_score)as int)as infrastructure_score,
 sum(case when Infrastructure_score <=33 then 1 else 0 end)as value_below_33,
 sum(case when Infrastructure_score > 33 and infrastructure_score<= 60 then 1 else 0 end)as value_between_33_60,
 sum(case when Infrastructure_score > 60 and infrastructure_score<= 75 then 1 else 0 end)as value_between_60_75,
 sum(case when Infrastructure_score >75 then 1 else 0 end)as value_above_75,school_management_type
...
select_query_state text:='select string_agg(col,'' '') from (select concat(select_query)as col from health_card_config where status=''true'' and category=''state_mgmt'' and time_period=''last30''  order by id)as d';   
...
Execute select_query_state into select_cols_state;
create_state_view=
'create or replace view health_card_index_state_mgmt_last30 as '||select_cols_state||'';
